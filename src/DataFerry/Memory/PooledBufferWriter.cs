...
public sealed class PooledBufferWriter<T> : IBufferWriter<T>, IDisposable
{
    ...
    public PooledBufferWriter(ArrayPool<T> pool)
    {
        _pool = pool;
    ...
    /// <summary>
    /// Returns the internal buffer to the pool and disposes of the <see cref="PooledBufferWriter{T}"/> instance.
    /// </summary>
    ...
    /// <summary>
    /// Resizes the internal buffer to accommodate at least the specified additional size hint.
    /// Calculates the new size, rents a new buffer from the pool, copies the existing data from the old buffer
    /// to the new one, and returns the old buffer to the pool if applicable.
    /// </summary>
    /// <param name="sizeHint">The minimum additional capacity required beyond the current index.</param>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if the required total size exceeds <see cref="Array.MaxLength"/>.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal void GrowBuffer(int sizeHint)
    {
        ...
        if ((uint)newSize > int.MaxValue)
        {
            ...
            ArgumentOutOfRangeException.ThrowIfGreaterThan(needed, (uint)Array.MaxLength, "Requested buffer size exceeds max length.");
