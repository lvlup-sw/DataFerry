...
/// </summary>
/// <remarks>
/// Heavily based on the implementations by the Caffeine and BitFaster libraries.
/// </remarks>
...
public class CountMinSketch<T> where T : notnull
{
    ...
    public CountMinSketch(int maximumSize)
    {
        ...
        if (typeof(T).IsPrimitive || typeof(T) == typeof(string))
        {
            ...
            _hashFunction = obj => obj.GetHashCode();
        }
        else
        {
            ...
            _hashFunction = obj => HashGenerator.GenerateHash(obj);
        }
    }
    ...
    public unsafe int EstimateFrequency(T value)
    {
        ...
        int blockHash = Spread(_hashFunction(value));
        int counterHash = Rehash(blockHash);

        // Calculate block index
        int block = (blockHash & _blockMask) << 3;
    ...
    private void EnsureCapacity(int maximumSize)
    {
        ...
        _table = new long[Math.Max(BitOperations.Log2((uint)maximum), 8)];
    ...
    private void Reset()
    {
        ...
        for (int i = 0; i < _table.Length; i += 4)
        {
            // Count the set bits in each element and add to the corresponding counter
            count0 += BitOperations.PopCount((ulong)(_table[i] & OneMask));
            count1 += BitOperations.PopCount((ulong)(_table[i + 1] & OneMask));
            count2 += BitOperations.PopCount((ulong)(_table[i + 2] & OneMask));
            count3 += BitOperations.PopCount((ulong)(_table[i + 3] & OneMask));

            // Right shift each element and apply the reset mask
            _table[i] = _table[i] >>> 1 & ResetMask;
            _table[i + 1] = _table[i + 1] >>> 1 & ResetMask;
            _table[i + 2] = _table[i + 2] >>> 1 & ResetMask;
            _table[i + 3] = _table[i + 3] >>> 1 & ResetMask;
        }
